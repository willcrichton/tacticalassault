...
if (CLIENT) then
	SWEP.PrintName 		= "FABARM SDASS Tactical shotgun"
...
SWEP.ViewModel 			= "models/weapons/v_shot_fabarm.mdl"
SWEP.WorldModel 		= "models/weapons/w_shot_fabarm.mdl"

SWEP.Primary.Sound 		= Sound("weapons/automa/auto.wav")
...
SWEP.IronSightsPos = Vector (2.0539, -2.1206, 1.4421)
SWEP.IronSightsAng = Vector (0.0153, -0.1405, 0)
...
function SWEP:Think()
	...
	if self.Weapon:GetNetworkedBool( "reloading") == true then
		...
		if self.Weapon:GetNetworkedInt( "reloadtimer") < CurTime() then
			...
			self.Weapon:EmitSound("weapons/fabarm/xm_cock.wav")
		...
		self.Weapon:EmitSound("weapons/fabarm/xm_insert.wav")
